function task3(decomp_times,SNR,type,wname,imageName)
...
var_noise = (std2(myImage - recon_image))^2;
SNR = 10*log10(var_s/var_noise);
SNR_orig = 10*log10(var_s/var_n);

%plot approximation, horizontal, vertical, and diagonal details for each
%decomposition level.
maxLevelToBePlotted=decomp_times*(decomp_times<8)+8*(decomp_times>8)

S = get(0,'MonitorPosition');
set(figure,'position',[S(3)*0.3/2 0 S(3)*0.7 S(4)]);

subplot(maxLevelToBePlotted+1,4,1)
imshow(recon_image,[]);
title(['Reconstructed Image, SNR = ',num2str(SNR),'dB']);
subplot(maxLevelToBePlotted+1,4,2)    
imshow(myImage,[]);
title('Original Image');
subplot(maxLevelToBePlotted+1,4,3)
imshow(myImage+eta,[]);
title(['Original Noise Image, SNR = ',num2str(SNR_orig),'dB']);

for k = 1:maxLevelToBePlotted
    subplot(maxLevelToBePlotted+1,4,4*k+1)
    imshow(a{k},[]);
    title(['Approximations']);
    ylabel(strcat("Level "+k))
    subplot(maxLevelToBePlotted+1,4,4*k+2)
    imshow(h{k},[]);
    title(['Horizontal Detail']);
    subplot(maxLevelToBePlotted+1,4,4*k+3)
    imshow(d{k},[]);
    title(['Diagonal Detail']);
    subplot(maxLevelToBePlotted+1,4,4*k+4)
    imshow(v{k},[]);
